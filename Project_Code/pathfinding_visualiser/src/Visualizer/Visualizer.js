...
import a_star from "./Path_finding_Algorithms/A_star_algo";
import bellman from "./Path_finding_Algorithms/BellmanFordAlgo";
import dijks from "./Path_finding_Algorithms/Dijkstras";
...
const algorithms = {
  ...
  a_star: { name: "A* Algorithm", algo: a_star },
  bell: { name: "Bellman Ford", algo: bellman },
  dijk: { name: "Dijkstra's Algo", algo: dijks },
};
const speeds = {
...
class Visualizer extends Component {
  ...
  componentDidMount = () => {
    const cells = [];
    // calculate how many rows and cols can be generated as per display size
    ...
    for (let row = 0; row < no_of_rows; row++) {
      ...
      for (let col = 0; col < no_of_cols; col++) {
        currentRow.push(nodes.EMPTY_NODE);
        currentRow_refers.push(React.createRef());
      }
      cells.push(currentRow);
      this.refers.push(currentRow_refers);
    }
    // Randomly initialize a start node
    let startNode = {
      r: Math.floor(Math.random() * no_of_rows),
      c: Math.floor(Math.random() * no_of_cols),
    };
    // Randomly initialize a end node
    let endNode = {
      r: Math.floor(Math.random() * no_of_rows),
      c: Math.floor(Math.random() * no_of_cols),
    };
    // End node should have different location then start node
    ...
    cells[startNode["r"]][startNode["c"]] = nodes.START;
    cells[endNode["r"]][endNode["c"]] = nodes.END;
